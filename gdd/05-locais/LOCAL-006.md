---
id: "LOCAL-006"
nome: "Cais do Lago Nobre"
status: "CANONICO"
imagem: "./assets/locais/LOCAL-006.png"
atualizado_por: "agente"
---

# Ficha de Local: Cais do Lago Nobre

## 1. Localização
- **Coordenadas:** `X ≈ 204 → 250, Y ≈ 5 → 15, Z ≈ 64 → 140` (Setor Nordeste)
- **Classificação:** Área Aquática & Pesca

## 2. Elementos
- Decks de madeira sobre a água com peixes animados nadando (`VidaDoLagoPeixes`).
- Barcos atracados, cabana de pescador e caixas de ferramentas.
- Posto de trabalho e descanso do Pescador Lucan.
